		...
		<BaseInspectorControls>
			<PanelBody title={ __( 'Settings', 'aquila-theme' ) }>
				<SelectControl
					...
					value={ mode }
					options={ [
						{ label: __( 'Automatic', 'aquila-theme' ), value: 'automatic' },
						{ label: __( 'Manual', 'aquila-theme' ), value: 'manual' },
					] }
					onChange={ ( newMode ) => {
						setAttributes( { mode: newMode } );
					} }
					help={ mode === 'automatic'
						? __( 'Automatically fetch latest posts based on criteria below', 'aquila-theme' )
						: __( 'Manually select posts to display', 'aquila-theme' )
					}
				/>
			</PanelBody>
			...
			{ mode === 'manual' && (
				<PanelBody title={ __( 'Manual Selection', 'aquila-theme' ) } initialOpen={ true }>
