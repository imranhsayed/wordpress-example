 ...
 *
 * Displays selected items that can be removed or reordered
 *
 * @param {Object} props - Component props
 * @param {Array} props.items - Selected items
 * @param {Function} props.onUpdated - Callback when items are reordered
	...
	return (
		<ul className="aquila-relationship__items">
			{ items.map( ( item, index ) => {
				return (
					<li key={ item.id || index } className="aquila-relationship__item">
						...
						<div className="aquila-relationship__item-action">
							<Tooltip text={ __( 'Remove', 'aquila-theme' ) }>
								<Button onClick={ () => onUnselected( item ) } icon="dismiss" />
							</Tooltip>
						</div>
					</li>
				);
