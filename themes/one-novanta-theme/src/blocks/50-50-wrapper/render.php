...
$image_alignment = $attributes['imageAlignment'] ?? 'left';
		...
		[
			'content' => Template::get_component(
				...
				[
					...
					'size'  => 'large',
					'attrs' => array(
						'class' => 'one-novanta-50-50-wrapper__image attachment-large size-large',
						'alt'   => esc_html__( 'Content Image', 'one-novanta-theme' ),
					),
				] 
			),
		]
	);
} elseif ( 'question' === $template ) {
	...
	$content_column = Template::get_component(
		...
		[
			'content' => implode(
				...
				array_filter(
					[
						$heading ? sprintf( '<h2 class="one-novanta-50-50-wrapper__heading has-xx-large-font-size">%s</h2>', esc_html( $heading ) ) : '',
					]
				)
			),
		]
...
$media_column = Template::get_component(
	'two-columns',
	'column',
	[
		'content' => $content,
...
$wrapper_attributes = get_block_wrapper_attributes( array( 'class' => sprintf( 'two-columns--media-align-%s', esc_attr( $image_alignment ) ) ) );
...
Template::render_component(
	...
	null,
	[
		'content'              => $content_column . $media_column,
