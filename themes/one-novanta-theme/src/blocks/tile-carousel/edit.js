...
export default function Edit( { attributes, setAttributes, clientId } ) {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Carousel Settings', 'one-novanta-theme' ) } className="tile-carousel-editor__settings-panel">
					<BaseControl __nextHasNoMarginBottom label={ __( 'Slides Management', 'one-novanta-theme' ) } id="slides-management">
						<Button
							variant="primary"
							onClick={ addSlide }
							...
							disabled={ ! editMode }
						>
							{ __( 'Add Carousel Slide', 'one-novanta-theme' ) }
						</Button>
						<p>{ __( 'Total Slides:', 'one-novanta-theme' ) } { totalSlides || 0 }</p>
					</BaseControl>

					<BaseControl __nextHasNoMarginBottom label={ __( 'Toggle Edit Mode', 'one-novanta-theme' ) } id="edit-mode-toggle">
						<Button
							variant="secondary"
						...
						>
							{ editMode
								? __( 'Disable Edit Mode', 'one-novanta-theme' )
								: __( 'Enable Edit Mode', 'one-novanta-theme' ) }
						</Button>
						{ ! editMode && <BaseControl label={ __( 'Please enable the edit mode, to continue editing the card.', 'one-novanta-theme' ) } __nextHasNoMarginBottom id="slides-management" className="tile-carousel-editor__settings-panel__media--info" /> }
					</BaseControl>
				</PanelBody>
			</InspectorControls>
			<div { ...blockProps }>
								...
								<div className="tile-carousel-editor__navigation">
									<div className="tile-carousel-editor__navigation-arrow tile-carousel-editor__navigation-arrow--previous">
										<button className="arrow-btn tile-carousel-editor__navigation-arrow-button">
											<img src={ previousSvg } alt={ __( 'Previous', 'one-novanta-theme' ) } />
										</button>
									</div>
									<div className="tile-carousel-editor__navigation-arrow tile-carousel-editor__navigation-arrow--next">
										<button className="arrow-btn tile-carousel-editor__navigation-arrow-button">
											<img src={ previousSvg } alt={ __( 'Next', 'one-novanta-theme' ) } />
										</button>
									</div>
								</div>
							)
