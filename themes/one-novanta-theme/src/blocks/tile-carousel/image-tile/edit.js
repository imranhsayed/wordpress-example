...
export default function Edit( {
	...
	const editMode = context[ 'one-novanta/editMode' ] ?? true;
	const imageRatio = context[ 'one-novanta/imageRatio' ] ?? '';
	const imageSize = context[ 'one-novanta/imageSize' ] ?? '';
	...
	useEffect( () => {
		if ( ! editMode && isSelected ) {
			...
			if ( parentBlockIds.length > 0 ) {
				...
				if ( parentBlockName === 'one-novanta/tile-carousel' ) {
		...
		<>
			...
			<div { ...blockProps }>
				<MediaPreview
					...
					imgAlt={ imageAlt || __( 'Image tile image', 'one-novanta-theme' ) }
				/>
					...
					{ imageURL && <>
						{ editMode ? (
							<RichText
								...
								placeholder={ __( 'Pre Heading', 'one-novanta-theme' ) }
